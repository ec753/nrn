...
,&_coef%d, _linmat%d);\n",
    ...
    if (method->subtype & DERF) { /* derivimplicit */
        Sprintf(buf,
                "%s %s%s_thread(%d, _slist%d, _dlist%d, _ml->contiguous_row(_iml).data(), %s, _ml->contiguous_row(_iml).data(), _ppvar, _thread, _nt);\n%s",
                deriv1_advance,
                ssprefix,
                method->name,
                numeqn,
...
, _linmat%d, _ml->contiguous_row(_iml).data(), _ppvar, _thread, _nt);\n",
                ssprefix,
                method->name,
                listnum,
                numeqn,
